...
const needsSVG = (statuImgJsValues) => (
    statuImgJsValues.HBUTTON || statuImgJsValues.MBUTTON
);
...
const generateTabletDescriptionFile = (vendor, firmware, values) => {
    ...
    const layout = needsSVG(values) ? `Layout=${getTabletId(vendor, values.ProductName)}.svg\n` : '';
    ...
    let tabletDescription =
        ...
        ${layout}Styli=@generic-no-eraser;
